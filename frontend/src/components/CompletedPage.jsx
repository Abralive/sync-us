...
export default function CompletedPage({ completed, activeUser, stardust, stats, onRefresh, embedded = false }) {
  ...
  function statusBadge(task) {
    if (task.stardust_awarded > 0) {
      return <span className="done-badge ok">{task.stardust_awarded} 星塵</span>;
    }
    if (!task.is_private && !task.confirmed) {
      if (task.completed_by_id === Number(activeUser)) {
        return <span className="done-badge wait">等對方確認</span>;
      }
      return (
        <button className="btn primary done-confirm" disabled={busyId === task.id} onClick={() => confirm(task.id)}>
          {busyId === task.id ? "確認中..." : "確認"}
        </button>
      );
    }
    return <span className="done-badge none">未入帳</span>;
  }
    ...
    <section className="done-page">
      {!embedded && (
        <div className="growth-intro">
          <span className="garden-kicker">完成</span>
          <h2>戳破紀錄</h2>
          <p>已完成的泡泡會放在這裡。</p>
        </div>
      )}
      ...
      <div className="done-summary">
        <div><strong>{completed.length}</strong><span>完成</span></div>
        <div><strong>{stardust}</strong><span>星塵</span></div>
      ...
      {list.length === 0 ? (
        <div className="empty">目前沒有符合條件的紀錄。</div>
      ) : (
            ...
            <article className="done-item" key={task.id}>
              <div className="done-item-main">
                ...
                <p className="task-detail dim">
                  {task.completed_by_name || "有人"} 在 {formatDateTime(task.completed_at)} 戳破
